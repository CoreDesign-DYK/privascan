        ...
        >
          ...
          <div className="flex flex-col h-full" style={{ width: `${100 / TOTAL_PAGES}%` }}>
            ...
            <div className="flex items-center gap-3 px-4 pt-6 pb-4 border-b border-gray-100 shrink-0">
              ...
              <p className="font-semibold text-[17px] text-gray-900 flex-1 text-center pr-8">Account Detail</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
              ...
              <div className="flex flex-col items-center gap-3 py-8 px-5 border-b border-gray-100">
                ...
                {user?.email && (
                  <p className="text-[13px] text-gray-500">{maskEmail(user.email)}</p>
                )}
              </div>

              {/* Signed-in method (read-only) */}
              <div className="px-5 py-4 border-b border-gray-100">
                <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wide mb-3">Signed in with</p>
                <div className="flex items-center gap-3">
                  {/* Provider icon */}
                  ...
                  {user?.provider === 'apple' && (
                    <div className="w-8 h-8 rounded-full bg-black flex items-center justify-center shrink-0">
                      <svg viewBox="0 0 24 24" className="w-4 h-4 fill-white" xmlns="http://www.w3.org/2000/svg">
                        <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.7 9.05 7.4c1.38.07 2.33.74 3.13.8 1.19-.24 2.33-.93 3.59-.84 1.54.12 2.7.72 3.46 1.83-3.18 1.9-2.43 5.76.52 6.87-.61 1.64-1.42 3.26-2.7 4.22zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/>
                      </svg>
                    </div>
                  )}
                  {user?.provider === 'email' && (
                    <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center shrink-0">
                      <svg viewBox="0 0 24 24" className="w-4 h-4 text-gray-600 stroke-current fill-none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <div>
                    <p className="text-[15px] text-gray-800 font-medium">
                      ...
                      {user?.provider === 'apple' && 'Apple ID'}
                      {user?.provider === 'email' && 'Email'}
                    </p>
                    {user?.email && (
                      <p className="text-[12px] text-gray-400 mt-0.5">{maskEmail(user.email)}</p>
                    )}
                  </div>
                </div>
              </div>
