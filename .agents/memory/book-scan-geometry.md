...
**How to apply:** Normalize portrait-locked sideways camera frames into a physical landscape coordinate system before Book detection: canvas-left must always be physical L and the binding must be vertical. Keep live guidance, auto-capture stability, final validation, gutter exclusion, page dewarping, DPI limits, and GPU/CPU fallbacks aligned to that geometry. Never project Preview geometry onto a still with potentially different framing; a fallback must use the exact frame from which its geometry was detected. Do not use low-resolution preview sharpness as a Book shutter gate. Apply interior page-surface correction only when multiple text-bearing regions provide coherent, bounded curvature evidence. A 2D correction field must preserve a positive vertical Jacobian and use the same inverse mapping on CPU and WebGL; otherwise retain the binding-curve warp.